...
- **FE Dev:** Frontend stack; SRS header `Frontend-Framework:`; when `multiple`, the task-surface/app mapping from SRS §3.4.2 and §3.4.5; the matching `fe-framework-coding-standard` reference path(s); for React Native / Flutter, the matching mobile companion skill paths for implementation plus conditional performance/platform-integration work; design system or component library if named; accessibility targets if stated; how to
