...
4. If the selected reference is React Native or Flutter, follow its mobile skill map:
   - always load the stack implementation skill;
   - load the stack performance skill only for large/slow/animated/image-heavy/startup/memory-sensitive work;
   - load the stack platform-integration skill only for permissions, native modules/plugins, deep links, app config, secure storage, lifecycle, or native iOS/Android files.
5. Inspect package files and source layout only as a consistency check. If source evidence disagrees with the SRS framework, halt and raise an SRS/code drift issue. Do not silently switch frameworks.
...
- Mobile stack skills: .claude/skills/react-native-implementation/SKILL.md, .claude/skills/react-native-performance/SKILL.md, .claude/skills/react-native-platform-integration/SKILL.md, .claude/skills/flutter-implementation/SKILL.md, .claude/skills/flutter-performance/SKILL.md, .claude/skills/flutter-platform-integration/SKILL.md
