...
4. Dispatch (Steps 4.5 availability, 4.6 journal+local detached worktree for code roles, 5 dispatch), then on each return: validate exit criteria and run the Step 7 finalization transaction: promote validated role-owned artifacts by path-scoped ingestion, apply `plan-update.json`, commit artifacts + master-plan transition together, record that full commit SHA as the journal's `finalized` marker, run the UI PIV gate (Step 7.5) where applicable, remove registered Git worktrees with `git worktree remove --force` or logical-role handoff-only directories with `rm -rf -- .worktrees/<role>-<task-id>/`, then delete the journal last.
