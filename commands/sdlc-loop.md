...
4. Dispatch (Steps 4.5 availability, 4.6 journal+local detached worktree for code roles, 5 dispatch), then on each return: validate exit criteria and run the Step 7 finalization transaction: promote validated role-owned artifacts by path-scoped ingestion, apply `plan-update.json`, commit artifacts + master-plan transition together, run the UI PIV gate (Step 7.5) where applicable, then clean up the worktree + journal.
