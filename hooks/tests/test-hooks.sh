...
run_exit "blocks Read on .ssh/config"       2 "$PRIVACY" '{"tool_name":"Read","tool_input":{"file_path":"./.ssh/config"}}'
run_exit "blocks Glob on .ssh"              2 "$PRIVACY" '{"tool_name":"Glob","tool_input":{"path":".ssh","pattern":"*"}}'
run_exit "allows Bash ssh -F .ssh/config"   0 "$PRIVACY" '{"tool_name":"Bash","tool_input":{"command":"ssh -F .ssh/config app01 uptime"}}'
run_exit "allows Bash rsync ssh config"     0 "$PRIVACY" "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"rsync -e 'ssh -F .ssh/config' build.tgz app01:/tmp/build.tgz\"}}"
run_exit "blocks Bash cat .ssh/config"      2 "$PRIVACY" '{"tool_name":"Bash","tool_input":{"command":"cat .ssh/config"}}'
run_exit "blocks Bash ssh printenv"         2 "$PRIVACY" '{"tool_name":"Bash","tool_input":{"command":"ssh -F .ssh/config app01 printenv"}}'
...
run_exit "blocks Read on .k8s/config"       2 "$PRIVACY" '{"tool_name":"Read","tool_input":{"file_path":".k8s/config"}}'
run_exit "blocks Read on kubeconfig.yaml"   2 "$PRIVACY" '{"tool_name":"Read","tool_input":{"file_path":"deploy/kubeconfig.yaml"}}'
run_exit "blocks Read on .k8s secret file"  2 "$PRIVACY" '{"tool_name":"Read","tool_input":{"file_path":".k8s/secrets.yaml"}}'
run_exit "allows Read on k8s manifest"      0 "$PRIVACY" '{"tool_name":"Read","tool_input":{"file_path":".k8s/deployment.yaml"}}'
run_exit "allows Bash kubectl kubeconfig"   0 "$PRIVACY" '{"tool_name":"Bash","tool_input":{"command":"kubectl --kubeconfig .k8s/config get pods -n app"}}'
run_exit "allows Bash KUBECONFIG env ref"   0 "$PRIVACY" '{"tool_name":"Bash","tool_input":{"command":"KUBECONFIG=.k8s/config kubectl get pods -n app"}}'
run_exit "blocks kubectl config view"       2 "$PRIVACY" '{"tool_name":"Bash","tool_input":{"command":"kubectl --kubeconfig .k8s/config config view --raw"}}'
run_exit "blocks Read on docker auth"       2 "$PRIVACY" '{"tool_name":"Read","tool_input":{"file_path":".docker/config.json"}}'
