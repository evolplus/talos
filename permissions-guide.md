...
| `orchestrator-bash-guard.cjs` | State-mutating Bash from Orchestrator context (installers, deploy/DB/HTTP mutations, destructive FS operations, in-place edits, risky Git, builds). The only destructive-FS carve-out is cleanup-only `rm`/`rmdir` when every target resolves strictly below `.worktrees/` or `.claude/dispatch-journal/`. Escape hatch: `CLAUDE_ALLOW_ORCHESTRATOR_BASH=1` |
