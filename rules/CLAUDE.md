...
- **No brownfield human-confirmation gate while `Extraction-Validated` is absent.** In brownfield onboarding (§12), SA `extract` and BA Mode E author the extracted architecture, contract stubs and SRS at `Source: extracted`; the independent `extraction-validator` is the sole writer of the `Extraction-Validated` stamp that unblocks Stage 4 (neither author may self-validate; the Orchestrator cannot stamp it; no operator override). It is the brownfield analogue of the SRS and architecture gates, and it closes the one remaining place where the kit let an agent approve its own work — `srs-source-validator` validates against `docs/requirements/`, which brownfield leaves empty, and `architecture-validator` presupposes a signed-off SRS. A `qualified` verdict certifies fidelity to the code only; the Stage 4 human gate still cannot be auto-approved. See `.claude/rules/brownfield-onboarding.md` §12 Stage 3.5 and `.claude/rules/sub-agent-registry.md` §3.12.
...
- **No worktree teardown before a verified promotion.** A dispatch is closed when its content is provably on main — `finalization.main_commit` is an ancestor of `HEAD` AND every path in `plan-update.json`'s `artifacts` manifest exists on `HEAD` and matches the worktree copy (§9 Step 7 step 6b). The finalization marker alone is not evidence: it proves *a* commit happened, not that it contained everything. Agent worktrees are created `--detach`, so removing one before promotion makes its commits unreachable with no ref to recover from. Promotion is path-scoped ingestion only; `git merge`/`cherry-pick`/`push` of worktree history is blocked by `local-worktree-git-guard.cjs` by design. `worktree-promotion-guard.cjs` blocks the teardown; `unpromoted-dispatch-audit.cjs` refuses to end a turn with a completed dispatch unpromoted. Code-role `plan-update.json` must carry the `artifacts` manifest. Full detail + escape hatches at `.claude/rules/worktree-isolation.md` §5 rules 5/5a/7 and `.claude/rules/hard-rules.md` § "Git + commit discipline".
