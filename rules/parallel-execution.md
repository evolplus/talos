...
- Once SRS is `Signed-off`: SA (architecture) and QA-Author in `by-us` mode may start together. QA-Author runs in **two passes**:
  ...
  - The Orchestrator detects the Pass 2 trigger by watching for `docs/instrumentation-contract.md` to be promoted to main while at least one open-issue with category `selector-pending-contract` exists. Dispatches QA-Author in `by-us` mode with `pass: 2` and the list of affected TC IDs.
...
|---|---|---|---|
| **A** | Figma URL present in requirements | PRE-sign-off surface qualification via UI/UX Designer `map` mode; POST-sign-off task handoff via `import` mode | `import` for each UI task; `incorporate` if approver edits Figma later. NO `create`. |
...
**Design-Flow A — Figma-provided path.** When a Figma URL exists in the requirements, BA Phase 1.X step 10 first requires UI/UX Designer `extract` mode so design content and design-token evidence enter the source corpus. BA then dispatches UI/UX Designer in `map` mode BEFORE setting SRS Status: Signed-off. The designer scans the Figma file, matches frames to SRS surfaces (US-by-US, FR-by-FR), pins Node IDs in SRS §3.4.1, and writes `docs/uiux/figma-mappings/v<srs-version>.md` with `Mapping-Status: qualified | gaps | orphans-only`. BA Phase 2 step 3.5 reads the mapping artifact and refuses to flip SRS Status to `Signed-off` while `Mapping-Status: gaps`.

Mapping qualifies that the provided Figma covers the SRS surfaces; it is not a task-scoped implementation handoff. After sign-off, every Flow A UI task still enters the design lifecycle at `import` mode. UI/UX Designer verifies the pinned frame's mapping signature, refreshes token evidence, exports checksummed full-frame references, and writes `docs/uiux/handoffs/<task-id>.md` with `## Reference Render`, `## Visual Composition Contract`, `## Asset Export Manifest`, and `## Design Element Manifest`. BA Phase 3 verifies identity, layout, assets, and element completeness before the Designated Design Approver confirms the Figma version. Only then may FE Dev start.
...
For Design-Flow A tasks, this lifecycle starts at `import` rather than `create`. Designs are mapped and pinned at SRS sign-off, but task-scoped handoff, Design Element Manifest, BA Phase 3 completeness, and approver confirmation still happen before FE Dev may start.
...
1. **Designer dispatch — Mode Selection.** The Orchestrator picks the Designer mode based on SRS `Design-Flow:` header + `## Design References` state + dispatch input. Six possible modes (see `.claude/agents/_templates/ui-ux-designer.md` for routing and `.claude/skills/figma-design-handoff/` / `.claude/skills/figma-srs-mapping/` / `.claude/skills/figma-requirements-extraction/` for procedures):
   ...
   | A (post-sign-off) | SRS `Signed-off`, mapping `qualified`, task lacks handoff | pinned Node IDs from map | `import` — read existing Figma and produce task-scoped handoff + manifest. |
...
5. **FE Dev** — May now start only after the task is `design-confirmed` and the handoff exists. FE Dev produces the per-task design contract (`docs/uiux/refs/<task-id>.md`) against the
   user-confirmed Figma file version. The contract includes reference-render identity, visual composition, asset export/use traces, `## Design Element Manifest`, and `## Implementation Trace Matrix`, so the exact frame, layout, every visual asset, and every Figma field/item/copy/action are tracked to code and test evidence. The `fe-dev-design-contract-guard.cjs` hook refuses any FE-source-code write from an `fe-dev` worktree until `docs/uiux/refs/<task-id>.md` exists, carries `Status: Frozen`, and contains those non-empty sections.
...
6. **Post-Implementation Verification (PIV) — symmetry to Step 3.** When FE Dev's `plan-update.json` proposes `to_status: ready-for-deploy` for a UI-bearing task, the kit verifies that the implementation matches the design-confirmed handoff BEFORE the Orchestrator commits the master-plan transition. Two enforcement layers:
   ...
   - **Runtime gate (`ui-task-readiness-guard.cjs`).** Refuses the `plan-update.json` write when any required UI artifact is absent or incomplete. Handoff and refs must contain reference-render, visual-composition, asset-export, and design-element evidence; refs must also be `Status: Frozen` with complete asset/composition/element trace matrices; visual specs must contain composition, asset, and element assertions; the by-task TC pack must contain at least one file. Triggered on `to_status: ready-for-deploy` for tasks where the task file shows `track: fe` / `be+fe`, or `Design sub-status:` set, or `Linked Surface:` non-null.
