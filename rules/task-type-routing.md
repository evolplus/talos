...
- **NO state-mutating Bash.** Any package installer, docker compose mutation, DB DML/DDL, HTTP mutation (curl POST/PUT/DELETE, etc.), FS destructive op, `sed -i`, redirect to source/docs/env/config paths, git push/pull/merge/cherry-pick/rebase/reset, chmod, build command, or service mutation is refused by `orchestrator-bash-guard.cjs` when run from main-repo cwd. Local worktree Git promotion/sync is additionally refused by `local-worktree-git-guard.cjs`. Deploy + build + integration work routes to DevOps / BE Dev / FE Dev / QA-Exec.
