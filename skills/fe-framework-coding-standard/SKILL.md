...
2. Read SRS §3.4.6 Environment Configuration before touching any API client, server URL, feature flag, analytics endpoint, or runtime config path. Use the declared frontend-owned key names and existing project config loader; if the task needs an endpoint/key that §3.4.6 does not declare, halt and raise an OQ instead of inventing a variable in code.
3. Map every changed screen/component to the task's SRS IDs, design handoff, Design Element Manifest, and visual spec. If the design is missing a required state, implement the required product behavior and record the design gap in the handoff or open-issues flow.
4. Build the screen/component from the manifest, not from memory of the screenshot:
