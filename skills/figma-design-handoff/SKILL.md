...
4. Pin resulting Node IDs back to SRS §3.4.1.
5. Write handoff rows with `Source: created`.
...
2. Capture structure, Auto Layout, components, variants, states, tokens, copy, and Figma metadata.
...
5. Write `Source: imported` handoff rows and a gap list.
6. Mark `Design-may-be-stale: yes` when Figma last-modified predates SRS `Last-Updated` by more than 30 days.
...
4. Regenerate the handoff and refs.
5. Leave unresolved items as open issues; do not mark design-ready while blocking gaps remain.
...
3. Regenerate handoff and refs without undoing human edits.
4. Add `## Human Edit Reconciliation Notes` for every changed node/property.
5. If human edits conflict with SRS requirements, do not silently fix them. Raise open issues for BA and the human approver.
